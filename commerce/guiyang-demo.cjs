...
// 城市演示档案预设：seed(pool) 缺省=贵阳（历史行为逐字节不变）；seed(pool,{city:'shenyang'})
// 出同构的沈阳演示批次（「盛京」虚构服务站）。资金隔离不认批次名，只认 initialization.mode='demo'
// （见 CLAUDE.md 规则 22 isDemo 泛化约定），新增城市预设时保持这条。
//   channelProducts=false：不同步 jz_products/jz_sku_workers——该城市频道货架由真实商家
//   seed 负责（如沈阳 scripts/life-shenyang-seed.cjs），避免「（演示）」商品混进频道列表。
const CITY_PRESETS={
 guiyang:{slug:'guiyang',label:'贵阳',brand:'筑城',batch:BATCH,travelSlug:'guiyang-demo-travel',channelProducts:true},
 shenyang:{slug:'shenyang',label:'沈阳',brand:'盛京',batch:'shenyang-life-demo-v1',travelSlug:'shenyang-demo-travel',channelProducts:false},
};
async function seed(pool,opts={}){
 const preset=CITY_PRESETS[opts.city||'guiyang'];if(!preset)throw Error('Unknown demo city preset: '+opts.city);
 const cfg={...preset};if(typeof opts.channelProducts==='boolean')cfg.channelProducts=opts.channelProducts;
 const c=await pool.getConnection(),result={batch:cfg.batch,city:cfg.slug,created:[],existing:[]};let locked=false;
 try{
  const [[lock]]=await c.query("SELECT GET_LOCK('"+cfg.slug+"_life_demo',20) acquired");if(!lock.acquired)throw Error('Demo initialization already running');locked=true;
  await c.beginTransaction();
  const [[city]]=await c.query('SELECT id,name,slug FROM cities WHERE slug=?',[cfg.slug]);if(!city)throw Error(cfg.label+' city missing');result.city=city;
  ...
  async function entity(kind,key,payload,scope){
   ...
   payload.initialization={...payload.initialization,batch:cfg.batch,mode:'demo',seed_key:seedKey,note:'用户授权的'+cfg.label+'演示资料，系统初始化；不代表商户签约、人工复核或真实服务承诺。'};
  ...
  for(const cat of categories){
   const vendorNo=cfg.batch+':'+cat.id;
   let [[vendor]]=await c.execute('SELECT id,name FROM jz_vendors WHERE vendor_no=?',[vendorNo]);
   if(!vendor){const name=cfg.brand+cat.name+'服务站（演示）';const [r]=await c.execute("INSERT INTO jz_vendors(type,name,logo,address,city_ids,phone,rating,review_count,badges,start_price,unit,hours,vendor_no,status,review_status,intro) VALUES(?,?,?,?,?,NULL,0,0,?,0,'起','09:00—18:00（演示）',?,'active','pending',?)",[cat.id,name,cat.icon,cfg.label+'演示服务区（虚构门店，无实际接待地址）',String(city.id),JSON.stringify(['演示商家']),vendorNo,'用于'+cfg.label+'本地生活频道的浏览、选品与预约流程演示；名称、门店及服务者均为虚构，不接受实际服务委托。']);vendor={id:r.insertId,name};result.created.push({kind:'channel_vendors',id:vendor.id});}else result.existing.push({kind:'channel_vendors',id:vendor.id});
   const merchant=await entity('merchants','merchant:'+cat.id,{name:vendor.name,vendor_id:vendor.id,city_id:city.id,contract_ref:'DEMO-NO-CONTRACT',contact:'演示服务台',phone:'未开通',description:cfg.label+cat.name+'演示商户；无真实合同、服务承诺或可拨打电话。'},{vendor_id:vendor.id});
   const store=await entity('stores','store:'+cat.id,{name:cfg.label+cat.name+'体验站（演示）',merchant_id:merchant.id,city_id:city.id,address:cfg.label+'演示服务区；非实际门店地址',phone:'未开通',capacity:30,lead_hours:0,description:'演示预约时段 09:00—18:00，每日30个演示名额，不派发真实服务。'},{vendor_id:vendor.id,merchant_id:merchant.id});
  ...
  async function voucher(sku,ref,price){
   ...
   return entity('skus','sku:'+sku.slug,{name:sku.name+' · 单品券（演示）',merchant_id:ref.merchant.id,store_id:ref.store.id,supply_minor:1,retail_minor:price,valid_days:90,description,conditions:'限'+cfg.label+'演示场景，一券一次；提前预约，90天有效。演示权益不兑付真实服务，不发生扣款或退款。',category_id:sku.category_id,channel_slug:sku.slug,initialization:{channel_sku_id:sku.id||null}},{vendor_id:ref.vendor.id,merchant_id:ref.merchant.id,store_id:ref.store.id});
  }
  if(cfg.channelProducts)for(const sku of skus){
   const ref=refs[sku.category_id];if(!ref)continue;
   const marker=cfg.batch+':sku:'+sku.id;
  ...
  if(!cfg.channelProducts)for(const sku of skus){
   // 频道货架不由本批次负责的城市：只发券，不落 jz_products（列表/详情走真实商家商品）。
   const ref=refs[sku.category_id];if(!ref)continue;
   if(Number(sku.price_from)>0&&!['insurance','consumer_finance'].includes(sku.category_id)){const v=await voucher(sku,ref,Math.round(sku.price_from*100));vouchers.set(sku.slug,{entity:v,ref});}
  }
  const travel=await voucher({name:'旅居体验服务',slug:cfg.travelSlug,spec:'一次旅居行程沟通与入住流程体验',category_id:'community'},refs.community,39900);vouchers.set(cfg.travelSlug,{entity:travel,ref:refs.community});
  ...
  for(const [key,topic]of Object.entries(topics)){
   ...
   await entity('packages','topic:'+key,{name:topic.title+' · 券包（演示）',city_id:city.id,price_minor:items.reduce((s,i)=>s+i.allocation_minor,0),description:'【'+cfg.label+'演示】'+topic.lede,items,topic_id:key,initialization:{topic_id:key,topic_url:'juzhu-topic.html?id='+key,related_services:topic.journey.filter(x=>!vouchers.has(x.sku)).map(x=>({name:x.title,slug:x.sku,category:x.cat}))}},{});
  }
  const giftItems=[['cleaning-daily-2h',19980],['appliance-repair',13320],['moving-city-standard',27750],[cfg.travelSlug,33300],['asset-appraisal',5550]].map(([slug,amount])=>item(slug,amount));
  const gift=await entity('packages','member-gift',{name:'新居住基础权益礼包（演示）',city_id:city.id,price_minor:99900,description:cfg.label+'演示版：保洁、维修、搬家、旅居、报告各一张，单券90天有效。',items:giftItems},{});
  await entity('plans','membership',{name:'新居住会员（演示）',city_id:city.id,package_id:gift.id,package_version:1,package:gift.payload,price_minor:99900,valid_days:365,description:'参考v1.9：999元/年，赠送五项独立生活权益。'+cfg.label+'无资金演示，不开通真实付费会员。'},{});
  await c.commit();return result;
 }catch(e){await c.rollback();throw e;}finally{if(locked)await c.query("SELECT RELEASE_LOCK('"+cfg.slug+"_life_demo')").catch(()=>{});c.release();}
}
module.exports={seed,isDemo,BATCH,CITY_PRESETS};
