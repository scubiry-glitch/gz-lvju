...
class Service {
 ...
 async promoterCatalog(city=''){const {isDemo}=require('./guiyang-demo.cjs');
  const commissionOf=(kind,p,demo)=>{
   if(demo)return {minor:0,bps:null};
   const src=kind==='skus'?(p.retail_minor?[{quantity:1,allocation_minor:p.retail_minor,rule:p.rule}]:[]):kind==='plans'?(p.package?.items||[]):(p.items||[]);
   if(!src.length)return {minor:null,bps:null};
   const price=kind==='skus'?p.retail_minor:p.price_minor;let income=0,known=true;
   for(const it of src){const r=it.rule||{},alloc=Number(it.allocation_minor)||0,qty=Number(it.quantity)||1;
    if(typeof r.beike_bps!=='number'||typeof r.channel_bps!=='number'){known=false;break;}
    income+=Math.floor(Math.floor(alloc*r.beike_bps/10000)*r.channel_bps/10000)*qty;}
   return {minor:known?income:null,bps:known&&price>0?Math.round(income/price*10000):null};
  };
  const out=[];for(const kind of ['packages','plans','skus']){const args=[kind];let where="e.status<>'archived'";if(city){where+=' AND (c.name=? OR c.slug=? OR CAST(c.id AS CHAR)=?)';args.push(city,city,city);}const rows=await this.get(this.pool,`SELECT e.id,e.city_id,e.published_version,v.snapshot,c.name city_name FROM commerce_${kind} e JOIN commerce_versions v ON v.kind=? AND v.entity_id=e.id AND v.version=e.published_version JOIN cities c ON c.id=e.city_id WHERE ${where}`,args);for(const e of rows){const p=parse(e.snapshot),demo=isDemo(p),comm=commissionOf(kind,p,demo);
   out.push({id:e.id,kind,city_id:e.city_id,city_name:e.city_name,version:e.published_version,name:p.name,description:p.description,price_minor:kind==='skus'?p.retail_minor:p.price_minor,valid_days:kind==='plans'?p.valid_days:null,is_demo:demo,category_id:p.category_id||null,topic_id:p.topic_id||p.initialization?.topic_id||null,channel_slug:p.channel_slug||null,redeem_channel:kind==='skus'?(p.redeem_channel||'offline'):null,exchange_tier:p.exchange_tier||null,exchange_tier_label:p.exchange_tier?(EXCHANGE_TIERS.find(t=>t.value===p.exchange_tier)||{}).label||null:null,exchange_tier_minor:p.exchange_tier_minor||null,items:(kind==='skus'?[{sku:p,quantity:1}]:(p.items||p.package?.items||[])).map(i=>({name:i.sku.name,quantity:i.quantity,description:i.sku.description,conditions:i.sku.conditions,valid_days:i.sku.valid_days,store_id:i.sku.store_id})),commission_minor:comm.minor,commission_bps:comm.bps});}}
  return out;}
