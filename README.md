...
新增：新居住券包与会员卡 **M1-A 正式业务底座**已接入测试站，包含正式账号、MySQL持久化及四端管理页面；真实支付保持关闭。入口、权限配置、部署及验收见 [M1-A交付说明](docs/verification/newliving-commerce-m1a/README.md)。

生活服务专题与会员初始化已落库：13个商家关联、6个券包、1个999元年会员方案，均为待完善草稿。[初始化清单与验收](docs/verification/newliving-commerce-initialization/README.md)。
