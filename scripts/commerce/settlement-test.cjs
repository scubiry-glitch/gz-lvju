...
(async()=>{
 ...
 try{
  ...
  await check('SC11 结算接口权限闸与四端可查',async()=>{
   ...
   const prec=await request('/promotion/records',actors.promoter);
   assert.equal(prec.status,200);
   assert((prec.body.data.redemptions||[]).some(r=>r.payout_status==='paid'&&Number(r.channel_minor)>0),'明细逐券可追溯代发到账状态');
   assert((prec.body.data.payouts||[]).some(p=>p.status==='paid'),'推广员可见代发到账记录');
