...
**确定性抽样**：`sampleHotels(roster, perTier=8)` —— **贵阳（演示城市，17 家）与沈阳（试点口径，9 家）名单门店全量入围**（`PINNED_CITIES`），其余名额按档位升序 → 品牌分层 → hotel_code 字典序轮转补足；每档总量仍为 8、合计 48 家。可重跑、可复算（隔离库断言），保证演示直链不失效。
