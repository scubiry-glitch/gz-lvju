...
- **沈阳可售商品**：列表/详情接口按「`jz_products.city_id` = 当前城市 + 商家 active 且 `city_ids` 命中」双维度过滤，**`city_id` 为 NULL 的商品在任何城市都不展示**。沈阳搬家用 `node scripts/moving-shenyang-seed.cjs seed|clean`（蓝犀牛 vendor 151，商品固定 id 段 5161-5166，幂等，clean 只删本脚本清单）；其余 11 个频道货架用 `node scripts/life-shenyang-seed.cjs seed|clean`（映射 jz_seed 标准商家 v2/v11/v31/v41-48，固定 id 段 5341+，幂等；**搬家故意不种**——混入其他商家会破坏「频道商品全部来自同一商家才显示 vendor_banner」规则）。生活权益演示批次同构扩展：`commerce/guiyang-demo.cjs` 的 `seed(pool,{city:'shenyang'})`（CITY_PRESETS，批次 `shenyang-life-demo-v1`、「盛京」虚构服务站、`channelProducts:false` 不落 jz_products），CLI `node scripts/commerce/seed-shenyang-demo.cjs --apply`；新增演示城市预设时保持 `initialization.mode:'demo'`（资金隔离只认这个，见规则 22）。
