...
- **逐晚计价（2026-09-10）**：订单金额 = 逐晚「日历覆盖价（`stay_calendar.price_night`，户型级 > 项目级）→ 默认夜价」合计，单一数据源 `stay_config.cjs stayNightPrices()`，与 C 端日历/下单页展示同口径；周末/节假日分档即按日期批量覆盖实现。下单占用把既有 open 行（可能带覆盖价）翻 `booked` 时**必须保留 `price_night`、落 `booking_id`**（`ON DUPLICATE KEY UPDATE` 勿再写 `status=status` 全列 no-op——那会让被覆盖晚不占房、可二次预订）；取消释放删除差异行，覆盖价随之清除须商家重设。0 元预订单（未配价）沿用旧口径放行，不加新闸。
