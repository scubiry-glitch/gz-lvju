...
- **最短连住**：`STAY_MIN_NIGHTS_DEFAULT`（rental=15 晚 / minsu=15 晚）+ `projects.ext.min_stay_nights`（rental 可配 1-365；minsu 可配 15-365，不得降到 15 晚以下）。**三处同口径校验**：C 端日历选段、下单页、`POST /api/juzhu/booking` 服务端兜底；改口径只改服务端常量或 ext，不要在前端另设数字。
