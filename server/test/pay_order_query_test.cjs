#!/usr/bin/env node
/**
 * server/test/pay_order_query_test.cjs
 * ---------------------------------------------------------------------------
 * 支付中台「支付订单查询」接口联调脚本（落兵台 id 1493269），底层走 thirdApi/payCenter.cjs 的 queryOrder。
 *
 * 环境：命令行第 1 参数优先；否则取环境变量 JUZHU_ENV / NODE_ENV（默认 test）。
 *   凭证可被 OAUTH_CLIENT_ID / OAUTH_CLIENT_SECRET 覆盖（生产建议仅用环境变量）。
 *
 * 用法：
 *   node server/test/pay_order_query_test.cjs [env] [appCode] [appOrderId]
 *   例如：node server/test/pay_order_query_test.cjs test bkjingjixueyuan 260423160401000203
 *         JUZHU_ENV=prod node server/test/pay_order_query_test.cjs '' bkxxx 2604...
 *   或作为模块：const { test } = require('./server/test/pay_order_query_test.cjs'); await test('prod', { appOrderId:'xxx' });
 * ---------------------------------------------------------------------------
 */
'use strict';
...
async function test(env, params = {}) {
  ...
  console.log(`[pay_order_query_test] env=${envName} → GET /pay/order/v2/query`, params);
  const json = await pay.queryOrder(params);
  console.log(JSON.stringify(json, null, 2));
  return json;
}

// 命令行直接运行：node server/test/pay_order_query_test.cjs [env] [appCode] [appOrderId]
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      console.error('[pay_order_query_test] 失败:', e.message);
      process.exit(1);
    }
  })();
}
