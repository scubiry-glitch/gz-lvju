...
async function seedAll(conn) {
  ...
  // 本地生活 7 频道：存量库也可增量 INSERT IGNORE
  const lifeCats = [
    ['telecom','电讯服务','📱',5],['insurance','财险服务','🛡',6],
    ['consumer_finance','消费金融','💳',7],['health_care','健康养老','🏥',8],
    ['home_maintain','居家维护','🏠',9],['asset','资产服务','🏦',10],['recycle','二手回收','♻️',11],
    ['community','社区服务','🏘',12],
  ];
  for (const [id, name, icon, ord] of lifeCats) {
    await conn.execute(
      'INSERT IGNORE INTO jz_categories(id,name,icon,sort_order,enabled) VALUES(?,?,?,?,1)',
      [id, name, icon, ord]
    );
  }
  // 同步补全原 4 类 icon（旧种子可能为 null）
  for (const [id, name, icon, ord] of [
    ['cleaning','保洁','🧹',1],['repair','维修','🔧',2],['moving','搬家','📦',3],['nanny','保姆','👶',4],
  ]) {
    await conn.execute(
      'INSERT IGNORE INTO jz_categories(id,name,icon,sort_order,enabled) VALUES(?,?,?,?,1)',
      [id, name, icon, ord]
  ...
  for (const [id, cat, name, slug, spec, price, unit, dur, ord] of lifeSkuRows) {
    await conn.execute(
  ...
  for (const [id, type, name, logo, address, rating, reviews] of lifeVendors) {
    await conn.execute(
      ...
      [id, type, name, logo, address, rating, reviews, jd(['whitelist']), id, now, now]
    );
  }
  const lifeProducts = [
    [4101,41,25,'宽带新装 · 千兆','装维上门',99],
    [4102,41,26,'号码携转 · 套餐','携号转网',0],
    // 注意：保险商品不再 seed 到 42（42 已被真实联调商家蓝犀牛占用），统一用 app.js 141+ 段商家 142
